...
{
    Vector4 norm_direction = direction.getNormalized();

    // Draw a normal with a fixed length of 0.15
    glBegin(GL_LINES);
    glVertex3dv(vertex.data);
    glVertex3dv((vertex + norm_direction * 0.1).data);
    glEnd();
    ...
    if (triangleVector.getMagnitude2() > 1.0e-6f)
    {
        ...
        glVertex3dv((vertex + norm_direction * 0.15).data);
        glVertex3dv((vertex + norm_direction * 0.1 - triangleVector).data);
        glVertex3dv((vertex + norm_direction * 0.1 + triangleVector).data);
        glEnd();
    }
}
