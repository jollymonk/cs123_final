...
SupportCanvas3D::SupportCanvas3D(QWidget *parent) : QGLWidget(QGLFormat(QGL::SampleBuffers), parent)
{
    ...
    //changed default settings to be less annoying on load
    m_defaultPerspectiveCamera->orientLook(Vector4(2, 2, 2, 0), Vector4(-1, -1, -1, 0), Vector4(0, 1, 0, 0));
...
void SupportCanvas3D::resetUpVector()
{
    ...
    m_defaultPerspectiveCamera->orientLook(m_defaultPerspectiveCamera->getPosition(), m_defaultPerspectiveCamera->getLook(), Vector4(0, 1, 0, 0));
    update();
}

void SupportCanvas3D::setCameraAxisX()
{
    m_defaultPerspectiveCamera->orientLook(Vector4(2, 0, 0, 0), Vector4(-1, 0, 0, 0), Vector4(0, 1, 0, 0));
    update();
}

void SupportCanvas3D::setCameraAxisY()
{
    m_defaultPerspectiveCamera->orientLook(Vector4(0, 2, 0, 0), Vector4(0, -1, 0, 0), Vector4(0, 0, 1, 0));
    update();
}

void SupportCanvas3D::setCameraAxisZ()
{
    m_defaultPerspectiveCamera->orientLook(Vector4(0, 0, 2, 0), Vector4(0, 0, -1, 0), Vector4(0, 1, 0, 0));
    update();
}
