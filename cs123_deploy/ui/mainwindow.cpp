...
#define SETUP_ACTION(dock, key) \
    ...
    SETUP_ACTION(emitterDock,   "CTRL+6");
...
#undef SETUP_ACTION
    ...
    tabifyDockWidget(ui->brushDock, ui->emitterDock);
    ui->brushDock->raise();
...
void MainWindow::renderImage()
{
    ...
    OpenGLScene *gl_scene = ui->canvas3D->getScene();
    if (gl_scene) {
        // TODO: Set up RayScene from glScene and call ui->canvas2D->setScene()

        Camera *camera = ui->canvas3D->getCamera();
        assert(camera);
        RayScene *ray_scene = new RayScene(gl_scene->getObjects(), gl_scene->getGlobal(), gl_scene->getLights(), camera);
        ui->canvas2D->setScene(ray_scene);
